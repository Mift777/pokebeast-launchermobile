...
public class ModloaderInstaller implements ModloaderDownloadListener {
    ...
    public void installLoaderFabric(String forgeVersion) {
        ...
        Runnable downloadTask = new FabriclikeDownloadTask(
                taskProxy,
                FabriclikeUtils.FABRIC_UTILS,
                versionParts[0],
                versionParts.length > 1 ? versionParts[1] : "latest",
                true
        );
        setTaskProxy(taskProxy);
        taskProxy.attachListener(this);
    ...
    public void onDataNotAvailable() {
        Tools.runOnUiThread(()->{
            getTaskProxy().detachListener();
            setTaskProxy(null);
            String message = loaderType == LoaderType.FABRIC
                    ? this.context.getString(R.string.fabric_dl_cant_read_meta, "Fabric")
